...
export const meta: MetaFunction = () => {
  return [
    { title: "Register Page" },
    { name: "description", content: "Register To Join our Family " },
  ];
};
const RegisterPage = () => {
  ...
  const onSubmitHandler = async(values:RegisterProps,helpers:FormikHelpers<RegisterProps>)=>{
        try {
          setLoading(true)
    ...
    >
      <Form className=' w-full lg:w-1/2 xl:w-1/3  mx-auto bg-white text-black py-10 px-2 rounded-md border border-yellow-400 font-pregular'>
          <div className="mb-3">
            ...
            <Field name="name" type="text" id="name" className="w-full py-3 px-2 border-b-black border-b  bg-gray-300 text-black outline-none focus:rounded-md focus:bg-gray-200" placeholder="Enter your name" />
            <ErrorMessage name='name' className='text-xs text-red-500' component={'p'} />
          </div>
          <div className="mb-3">
            <label htmlFor="email">Email</label>
            <Field name="email" type="email" id="email" className="w-full py-3 px-2 border-b-black border-b  text-black bg-gray-300 outline-none focus:rounded-md focus:bg-gray-200" placeholder="Enter your Email Address" />
